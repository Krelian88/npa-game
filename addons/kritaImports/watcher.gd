	...
	if _is_debug_enabled():
		...
		match mode:
			ScanMode.OPEN_SCENE_ONLY:
				print("Krita Watcher enabled - Open scene only (FAST)")
			ScanMode.ALL_SCENES:
				print("Krita Watcher enabled - Full project scan (SLOW)")
			ScanMode.DISABLED:
				print("Krita Watcher enabled - Scanning disabled")

func _exit_tree():
	if filesystem:
		filesystem.resources_reimported.disconnect(_on_resources_reimported)
	if _is_debug_enabled():
		print("Krita Watcher disabled")
...
func _on_resources_reimported(resources: PackedStringArray):
	...
	for resource_path in resources:
		if resource_path.ends_with(".kra"):
			if _is_debug_enabled():
				print("========================================")
				print("Krita file reimported: ", resource_path)
			_update_all_project_references(resource_path)
			if _is_debug_enabled():
				print("========================================")

func _update_all_project_references(kra_path: String):
	var new_img = ResourceLoader.load(kra_path, "", ResourceLoader.CACHE_MODE_REPLACE)
	if not new_img or not new_img is Image:
		push_error("Failed to load updated Krita image")
		return
	
	var scan_mode = _get_scan_mode()
	
	var open_scene_count = _update_open_scene(kra_path, new_img)
	if _is_debug_enabled():
		print("Updated %d nodes in open scene" % open_scene_count)
	
	if scan_mode == ScanMode.OPEN_SCENE_ONLY:
		if _is_debug_enabled():
			print("✓ Open scene only mode - skipping project scan")
		return
	
	if _is_debug_enabled():
		print("Scanning entire project for references to this Krita file...")
	...
	if scan_mode == ScanMode.ALL_SCENES:
		...
		for scene_path in scene_files:
			...
			var result = _update_scene_file(scene_path, kra_path, new_img)
			if result > 0:
				scenes_updated += 1
	...
	if _should_scan_materials():
		...
		for material_path in material_files:
			if _update_material_file(material_path, kra_path, new_img):
				materials_updated += 1
...
func _update_open_scene(kra_path: String, new_img: Image) -> int:
	...
	var count = _update_nodes_recursive(edited_scene, kra_path, new_img)
...
func _update_scene_file(scene_path: String, kra_path: String, new_img: Image) -> int:
	...
	var count = _update_nodes_recursive(scene_root, kra_path, new_img)
...
func _update_material_file(material_path: String, kra_path: String, new_img: Image) -> bool:
	...
	if material is StandardMaterial3D:
		updated = _update_standard_material_3d(material, kra_path, new_img)
	elif material is ShaderMaterial:
		updated = _update_shader_material(material, kra_path, new_img)
...
func _update_standard_material_3d(material: StandardMaterial3D, kra_path: String, new_img: Image) -> bool:
	var updated = false
	...
	for prop in texture_properties:
		...
		if texture and _should_update_texture(material, prop, kra_path):
			material.set(prop, new_texture)
			updated = true
	
	return updated

func _update_shader_material(material: ShaderMaterial, kra_path: String, new_img: Image) -> bool:
	var updated = false
	...
	for param in params:
		if param.type == TYPE_OBJECT and param.hint == PROPERTY_HINT_RESOURCE_TYPE:
			...
			if param_name.begins_with("shader_parameter/"):
				...
				if value is Texture2D and _should_update_texture(material, param_name, kra_path):
					material.set_shader_parameter(actual_param, new_texture)
					updated = true
	
	return updated

func _should_update_texture(resource: Resource, property: String, kra_path: String) -> bool:
	var meta_key = "kra_source_" + property
	if resource.has_meta(meta_key):
		return resource.get_meta(meta_key) == kra_path
	
	var texture = resource.get(property)
	if texture:
		if texture.resource_path == kra_path:
			resource.set_meta(meta_key, kra_path)
			return true
		
		if texture is ImageTexture:
			var img = texture.get_image()
			if img and img.has_meta("kra_source") and img.get_meta("kra_source") == kra_path:
				resource.set_meta(meta_key, kra_path)
				return true
	
	return false

func _update_nodes_recursive(node: Node, kra_path: String, new_img: Image) -> int:
	var count = 0
	var new_texture = ImageTexture.create_from_image(new_img)
	...
	if node is TextureRect:
		...
		if _update_animated_sprite(node, kra_path, new_texture):
			count += 1
	elif node is TextureButton:
		for prop in ["texture_normal", "texture_pressed", "texture_hover", "texture_disabled", "texture_focused"]:
			if _update_node_texture_property(node, prop, kra_path, new_texture):
				count += 1
		...
		if node.process_material:
			if _update_particle_material(node.process_material, kra_path, new_img):
				count += 1
		...
		for i in range(node.get_surface_override_material_count()):
			...
			if mat:
				if mat is StandardMaterial3D:
					if _update_standard_material_3d(mat, kra_path, new_img):
						count += 1
				elif mat is ShaderMaterial:
					if _update_shader_material(mat, kra_path, new_img):
						count += 1
	elif node is CSGMesh3D:
		if node.material:
			if node.material is StandardMaterial3D:
				if _update_standard_material_3d(node.material, kra_path, new_img):
					count += 1
	
	if texture_property != "":
		if _update_node_texture_property(node, texture_property, kra_path, new_texture):
			count += 1
	
	for child in node.get_children():
		count += _update_nodes_recursive(child, kra_path, new_img)
...
func _update_node_texture_property(node: Node, property: String, kra_path: String, new_texture: ImageTexture) -> bool:
	...
	if _is_debug_enabled():
		...
		print("    Looking for kra path: '%s'" % kra_path)
	
	var meta_key = "kra_source_" + property
	if node.has_meta(meta_key):
		if _is_debug_enabled():
			print("    Node already has tag: ", node.get_meta(meta_key))
		if node.get_meta(meta_key) == kra_path:
			node.set(property, new_texture)
			if _is_debug_enabled():
				print("    ✓ Updated from existing tag")
			return true
	
	if texture_path == kra_path:
		node.set_meta(meta_key, kra_path)
		node.set(property, new_texture)
		if _is_debug_enabled():
	...
	if texture is ImageTexture:
		...
		if img:
			...
			if img.has_meta("kra_source"):
				var source_path = img.get_meta("kra_source")
				if _is_debug_enabled():
					print("    Image has kra_source metadata: ", source_path)
				if source_path == kra_path:
					node.set_meta(meta_key, kra_path)
					node.set(property, new_texture)
					if _is_debug_enabled():
	...
	if texture_path.begins_with("res://") and texture_path.contains("::"):
		...
		var all_possible_source_tags = ["psd_source_" + property, "png_source_" + property, "jpg_source_" + property]
		...
		var import_path = kra_path + ".import"
					...
					if imported_img and imported_img is Image and texture is ImageTexture:
						...
						if current_img:
							if imported_img.get_width() == current_img.get_width() and imported_img.get_height() == current_img.get_height():
								...
								print("    Dimensions don't match: kra=%dx%d vs current=%dx%d" % [
...
func _update_particle_material(material: Material, kra_path: String, new_img: Image) -> bool:
	if material is ShaderMaterial:
		return _update_shader_material(material, kra_path, new_img)
	return false

func _update_animated_sprite(node: Node, kra_path: String, new_texture: ImageTexture) -> bool:
	# AnimatedSprite2D and AnimatedSprite3D use SpriteFrames
	...
	for anim_name in animations:
		...
		for i in range(frame_count):
			...
			if frame_texture:
				# Check if this frame matches our kra file
				var meta_key = "kra_source_frame_%s_%d" % [anim_name, i]
				
				# Check existing metadata
				if sprite_frames.has_meta(meta_key):
					if sprite_frames.get_meta(meta_key) == kra_path:
						sprite_frames.set_frame(anim_name, i, new_texture)
						updated = true
						if _is_debug_enabled():
							print("    ✓ Updated frame %d in animation '%s'" % [i, anim_name])
				# Check if texture path matches
				elif frame_texture.resource_path == kra_path:
					sprite_frames.set_meta(meta_key, kra_path)
					sprite_frames.set_frame(anim_name, i, new_texture)
					updated = true
					if _is_debug_enabled():
...
func _images_match(img1: Image, img2: Image) -> bool:
...
# Helper functions to access project settings
func _get_scan_mode() -> int:
	return ProjectSettings.get_setting("kra_importer/scan_mode", 0)

func _should_scan_materials() -> bool:
	return ProjectSettings.get_setting("kra_importer/scan_materials", false)

func _is_debug_enabled() -> bool:
	return ProjectSettings.get_setting("kra_importer/debug_prints", false)
